# AI 开发操作系统（vibe_workflow）- AGENTS.md

本文件是这个仓库（以及未来所有迁移出去的项目模板）的**顶层行为契约**：给 AI 用，不是写给人看的“说明书”。

目标：让 AI 在任何项目里稳定执行全流程：**规划 → 开发 → 测试 → 修复 → 上线**。

> 说明：本仓库将 `AGENTS.md` 作为“单一真相源”。
> - `CLAUDE.md` 仅作为 Claude Code 的入口文件，会指向并要求优先阅读本文件。
> - 如需修改规则，请优先修改 `AGENTS.md`，再同步 `CLAUDE.md`（保持一致）。
...
### 0.1 三条硬约束（不可妥协）
- 不许误改/不许越界：所有改动必须可追溯（至少输出 `git diff`/变更摘要/影响范围）。
- 不许跳过测试/验证：测试失败必须停机或回滚；禁止“通过删测试/关校验”来过关。
- 不许瞎猜：关键假设必须向用户确认；缺信息就问。

### 0.2 权限策略（全开，但用流程收敛风险）
- 默认允许：读写文件、运行命令、联网（如启用 MCP）。
- 关键动作必须显式门禁：`/vibe/apply`、`/vibe/deploy`、`/vibe/release`、`/vibe/db-migrate`。
  - 未显式触发门禁命令时：只允许做信息收集、方案设计、以及“可回滚的最小变更试验”。

### 0.3 多 Agent 协作（推荐最小角色集）
- `planner`：澄清需求、拆解任务、定义验收与风险；不直接改代码。
- `executor`：按计划实现；最小改动；失败立即止血。
- `reviewer`：审 diff、审边界、审回归风险。
- `test-engineer`：负责验证与失败诊断，产出复现步骤与最小用例。
- `release-engineer`：负责构建/部署入口脚本化、回滚策略、发布检查。

---

## 1. 目录约定（vibe_workflow 仓库）

- `plans/`：你的“方法论/范式/策略”资产（中英双语）。
  - `plans/ai-dev-paradigm-plan.zh.md`
  - `plans/ai-dev-paradigm-plan.en.md`
- `mcp/`：MCP 配置片段（OpenCode JSONC + Codex TOML 示例）。
  - `mcp/opencode.mcp.example.jsonc`
  - `mcp/codex.config.toml.example`
- `Makefile` + `scripts/`：构建/测试/部署入口（默认 `make deploy` 调用 `scripts/deploy.sh`）。
- `.sisyphus/`：oh-my-opencode 编排运行目录（Prometheus plans/drafts 与 /start-work 状态）。
- `vibe-coding-cn/`：Vibe Coding 的知识库与提示词资产（含 system prompts）。
- `learn-opencode/`：OpenCode 使用与工作流教程。
- `oh-my-opencode/`：多 Agent 编排与 Category+Skill 体系（可作为主编排器）。
...
### 2.2 前端（React/Vue，优先 Ant Design Pro）
- 包管理器：`yarn`
- Ant Design Pro（Umi Max）典型流水线：
  - 开发：`yarn start:dev`（建议默认无 mock）
  - 质量门禁：`yarn lint`（Biome + `tsc --noEmit`）
  - 构建：`yarn build` → 产物 `dist/`
  - 预览：`yarn preview`
...
无论是后端/前端/全栈/内嵌 AI，统一遵循：
1) 先明确目标、非目标、验收方式（planner）。
2) 再实现最小变更（executor）。
3) 再验证（test-engineer），失败则止血或回滚。
4) 再交付（release-engineer）。

任何时候只要出现不确定：停下、提问、再继续。

---

## 4. 关键：融合 Vibe Coding 系统提示词（v10）

你要求将 `vibe-coding-cn/i18n/zh/prompts/01-系统提示词/AGENTS.md/10/CLAUDE.md` 的内容**完整包含并融合**。

融合方式：
- 上半部分（0~3 章）已经把最重要的“硬约束/角色/流程”以**可执行规则**形式前置。
- 下方附录保留 v10 **全文原样**，作为：
  - AI 行为的“完整规格书”
  - 审计与回溯依据
...
## 5. 附录：Vibe Coding System Prompt v10（全文）

> 来源：`vibe-coding-cn/i18n/zh/prompts/01-系统提示词/AGENTS.md/10/CLAUDE.md`
...
注释与命名：  
- 注释、文档、日志文案使用中文  
- 除对人可见文本外，其他（变量名、类名、函数名、模块名等）统一使用英文
固定指令：  
